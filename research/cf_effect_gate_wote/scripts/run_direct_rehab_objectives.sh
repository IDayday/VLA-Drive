...
RUN_ROOT="${RUN_ROOT:-experiments/cf_effect_wote_direct_rehab/direct-rehab-v1-20260901/objective-ablation-v1}"
...
if [[ -e "${RUN_ROOT}" ]]; then
  echo "refusing existing objective-ablation output: ${RUN_ROOT}" >&2
  exit 2
fi
mkdir -p "${RUN_ROOT}"

for objective in O0 O1 O2 O3; do
