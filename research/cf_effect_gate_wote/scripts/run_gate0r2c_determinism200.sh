...
ten_summary="$six_factor_report_dir/g0r2b_summary.json"
run1="$six_factor_output_root/g0r2c-run1"
run2="$six_factor_output_root/g0r2c-run2"

cf_gate_run_python - "$ten_summary" <<'PY'
import json, sys
value = json.load(open(sys.argv[1], encoding="utf-8"))
if value.get("status") != "TEN_SCENE_DETERMINISM_PASS":
    raise SystemExit("G0-R2c is NOT_RUN because G0-R2b did not pass")
PY
actual_tokens_sha="$(sha256sum "$tokens" | awk '{print $1}')"
[[ "$actual_tokens_sha" == "$six_factor_expected_tokens_sha" ]] || {
  printf 'fixed token SHA256 mismatch: %s\n' "$actual_tokens_sha" >&2; exit 3;
}
[[ ! -e "$run1" && ! -e "$run2" ]] || { printf '%s\n' 'refusing existing G0-R2c run' >&2; exit 3; }
for run in "$run1" "$run2"; do
  cf_gate_run_python -m research.cf_effect_gate_wote.src.independent_relabel run-six-factor \
    ...
    --expected-scenes 200 --shard-scenes 16
done
cf_gate_run_python -m research.cf_effect_gate_wote.src.six_factor_verdict compare \
  --run1 "$run1" --run2 "$run2" --expected-scenes 200 \
  --pass-status SIX_FACTOR_RELABEL_PASS \
  --output-csv "$six_factor_report_dir/relabel_consistency_200.csv" \
  --output-summary "$six_factor_report_dir/relabel_consistency_200_summary.json"
