...
if [[ "$variant" == D || "$variant" == E || "$variant" == E_adapter ]]; then
  extra+=(--provider-checkpoint "$WORLD_ARTIFACTS/provider_dense_isolation/checkpoint.pt")
fi
exec "$WORLD_PYTHON" -u tools/structured_world/train.py \
 ...
 --target-cache "${WORLD_TARGET_CACHE:-$WORLD_ARTIFACTS/targets_v6_train8192}" --config "configs/structured_world_v1/$variant.yaml" \
