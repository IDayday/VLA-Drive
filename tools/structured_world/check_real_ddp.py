...
def main():
 ...
 p.add_argument('--run-id',default='real_ddp_resume_v4');p.add_argument('--joint-action',action='store_true')
 a=p.parse_args();dist.init_process_group('nccl');rank=dist.get_rank();torch.cuda.set_device(rank)
 if rank==0:reserve(a.ledger,a.run_id,3,vars(a))
 ...
 cfg={'enabled':True,'agent_tokens':32,'lambda_ego':1. if a.joint_action else 0.,'lambda_cls':1.,'lambda_box':1.,'lambda_motion':1.,'historical_auxiliary_losses':False}
 if a.joint_action:agent.model.action_model.requires_grad_(True)
 ...
 if rank==0:record(a.ledger,a.run_id,1)
 out=Path(a.output);out.mkdir(parents=True,exist_ok=True)
 state={k:v.detach().cpu().clone() for k,v in policy.state_dict().items() if not k.startswith('baseline.') or (a.joint_action and k.startswith('baseline.action_model.'))}
 ...
 if rank==0:record(a.ledger,a.run_id,2)
 reference={k:v.detach().clone() for k,v in policy.state_dict().items() if not k.startswith('baseline.') or (a.joint_action and k.startswith('baseline.action_model.'))}
 clone=StructuredWorldPolicy(copy.deepcopy(agent.model) if a.joint_action else agent.model,cfg).cuda().eval()
 ...
 if a.joint_action:clone.baseline.action_model.load_state_dict({k.removeprefix('baseline.action_model.'):v for k,v in loaded['state'].items() if k.startswith('baseline.action_model.')},strict=True)
