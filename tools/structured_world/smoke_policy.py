...
def main():
 p=argparse.ArgumentParser();p.add_argument('--checkpoint',required=True);p.add_argument('--vlm',required=True);p.add_argument('--data-root',required=True);p.add_argument('--manifest',required=True);p.add_argument('--output',required=True);p.add_argument('--split',default='train');p.add_argument('--seed',type=int,default=20260926);p.add_argument('--limit',type=int,default=1);a=p.parse_args()
 ...
 policy=StructuredWorldPolicy(agent.model,{'enabled':False}).cuda().eval()
 ...
 for i in range(len(ds)):
  ...
  disabled=policy.predict_action([example])['normalized_actions']
  np.testing.assert_array_equal(disabled,arrays['normalized_actions'])
  policy.world_enabled=True
  with torch.autocast('cuda',dtype=torch.bfloat16):
   conditions,prediction=policy.encode_conditions([example])
   objective=prediction['boxes'].float().square().mean()+prediction['future_xy'].float().square().mean()
  objective.backward()
  gradients={name:float(param.grad.float().norm()) for name,param in policy.named_parameters() if param.requires_grad and param.grad is not None}
  assert gradients['reader.queries'] > 0
  assert not any(param.grad is not None for param in agent.model.parameters())
  (out/'gradients.json').write_text(json.dumps(gradients,indent=2))
  np.savez(out/f'{example["token"]}.npz',**arrays)
  policy.world_enabled=False
