...
def main():
 ...
 policy=StructuredWorldPolicy(agent.model,yaml.safe_load(Path(a.config).read_text())).cuda().eval();load_delta(policy,a.delta);policy.requires_grad_(False)
 ...
 inputs,_=load_world_batch(examples[:1],a.target_cache,load_targets=False);provider=policy.provider;provider.bind_cache_identity()
 ...
 report={'status':'PASS','online_cached_external_action_max_abs':0,'current_only_without_target_loading':True,'rejected':rejected,'real_two_scene_repeat_alignment':True,'repeats':repeats,'zero_gate_gradient':gate,'opened_adapter_branch_gradient':branch,'provider_metadata':meta}
