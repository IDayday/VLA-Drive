...
def main():
    ...
    if rank==0:
        Path(a.output).write_text(json.dumps({'status':'PASS','device':'2 real CUDA GPUs NCCL','empty_rank':0,'max_abs_gradient_difference':error,'scope':'world heads/loss reduction; not full Qwen DDP or exact resume'},indent=2))
