...
mkdir -p "${CACHE_DIR}/model_candidates/scoring_logs"

run_one() {
  local shard="$1"
  local padded
  padded=$(printf '%04d' "${shard}")
  "${PYTHON_BIN}" -m tools.shared_future_candidate_consequence.build_model_candidate_bank \
    --mode shard \
    --split "${SPLIT}" \
    ...
    --cache-dir "${CACHE_DIR}" \
    >"${CACHE_DIR}/model_candidates/scoring_logs/shard_${padded}.log" 2>&1
}
...
if (( failures > 0 )); then
  echo "${failures} model-bank scoring shard(s) failed; inspect ${CACHE_DIR}/model_candidates/scoring_logs" >&2
  exit 1
fi
