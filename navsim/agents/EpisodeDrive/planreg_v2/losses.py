...
def global_valid_mean(values,valid):
    ...
    denominator = valid.sum().to(values.dtype)
    world = 1
...
def trajectory_stage_loss(proposals,target,valid,std):
    """Physical WTA followed by sum-coordinate/time-mean scaled regression."""
    error = proposals-target[:,None]
    ...
    return global_valid_mean(value,valid.any(-1))


def trajectory_loss(stages,targets,std,weights=(.25,.25,.25,.25),long_weight=1.):
    losses = []
...
def world_model_loss(tf,ro,targets,token_valid,future_valid,action_valid):
    target = state_norm(targets).detach()
    ...
    def branch(pred,valid):
        ...
        return global_valid_mean(error,valid),[global_valid_mean(error[:,i],valid[:,i]) for i in range(3)]
    tf_loss,tf_each = branch(tf,tf_valid)
    ro_loss,ro_each = branch(ro,ro_valid)
