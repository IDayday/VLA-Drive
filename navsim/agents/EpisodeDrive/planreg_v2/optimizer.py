import math
import torch
from . import RECIPE_VERSION, SCHEDULE_VERSION

PEAK_LRS = dict(planning_adapter=2e-4,fusion=2e-4,action_generator=2e-4,scorer=2e-4,
                future_predictor=2e-4,semantic_queries=1e-4,vision_qv_lora=4e-5,language_lora=1e-5)
CAPS = dict(planning_adapter=3e-4,fusion=3e-4,action_generator=3e-4,scorer=3e-4,
            future_predictor=3e-4,semantic_queries=1.5e-4,vision_qv_lora=5e-5,language_lora=1e-5)


def resolve_learning_rates(actual_global_batch,reference_lrs=None):
    if actual_global_batch<1: raise ValueError('Positive actual world*microbatch*accumulation required')
    references=dict(PEAK_LRS,**(reference_lrs or {}))
    if set(references)!=set(CAPS): raise ValueError('Unknown or obsolete LR group name')
    if references['language_lora']!=1e-5: raise ValueError('Language LoRA is fixed at 1e-5')
    scale=math.sqrt(actual_global_batch/32.)
    return {name:dict(reference_global_batch=32,actual_global_batch=actual_global_batch,
        reference_lr=lr,cap=CAPS[name],scale=1. if name=='language_lora' else scale,
        resolved_peak=1e-5 if name=='language_lora' else min(lr*scale,CAPS[name]),
        formula='fixed' if name=='language_lora' else 'min(reference_lr*sqrt(actual_global_batch/32),cap)',
        recipe_version=RECIPE_VERSION,schedule_version=SCHEDULE_VERSION) for name,lr in references.items()}
...
def logical_group(name):
    ...
    if name.startswith('backbone.task_queries.'): return 'semantic_queries'
    ...
    if name.startswith(('action_head.hist_encoding.','action_head.init_feature.','action_head.attention.','action_head.trajectory_head.')): return 'action_generator'
...
def build_optimizer(agent,total_steps,learning_rates=None):
    resolution=resolve_learning_rates(getattr(agent,'config',{}).get('global_batch',32),learning_rates)
    rates={n:v['resolved_peak'] for n,v in resolution.items()}
    groups = {}
    no_decay_ids = set()
    ...
    summary = [dict(name=n,parameters=sum(p.numel() for p in g['params']),lr=g['lr'],weight_decay=g['weight_decay'],
                    **resolution[g['logical_name']]) for n,g in sorted(groups.items())]
