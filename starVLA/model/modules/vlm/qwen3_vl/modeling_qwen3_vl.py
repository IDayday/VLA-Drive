...
class Qwen3VLVisionModel(Qwen3VLPreTrainedModel):
    ...
    def vit_tokens_to_featmap(
        self,
        hidden_states: torch.Tensor,  # [seq_len, C]
        grid_thw: torch.Tensor,       # [1, 3] = [1, H, 3W]
        num_views=3, view_order=(1,0,2)
    ):
        s = self.spatial_merge_size
        tokens_per_img = (grid_thw[:,0] * (grid_thw[:,1]//s) * (grid_thw[:,2]//s)).tolist()
        chunks = torch.split(hidden_states, tokens_per_img, dim=0)

        feat_list = []
        for tok, (t,h,w) in zip(chunks, grid_thw.tolist()):
            Hm, Wm = h//s, w//s
            feat = tok.view(Hm, Wm, -1).permute(2,0,1).contiguous()  # [C,Hm,Wm]
            feat_list.append(feat)

        feat = torch.stack(feat_list, dim=0)  # [num_images,C,Hm,Wm]
        assert feat.shape[0] % num_views == 0
        B = feat.shape[0] // num_views
        feat = feat.view(B, num_views, *feat.shape[1:])            # [B,3,C,Hm,Wm]

        feat = feat[:, list(view_order)]                           # reorder to l,f,r
        flr = torch.cat([feat[:,i] for i in range(num_views)], dim=-1)  # [B,C,Hm,3Wm]
        return flr, feat
