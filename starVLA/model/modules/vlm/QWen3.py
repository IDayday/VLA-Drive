...
from starVLA.model.modules.vlm.qwen3_vl.modeling_qwen3_vl import Qwen3VLForConditionalGeneration
...
class _QWen3_VL_Interface(nn.Module):
    ...
    def __init__(self, config: Optional[dict] = None, **kwargs):
        ...
        print("loaded name_or_path:", model.config._name_or_path)
        print("vocab/emb size:", model.get_input_embeddings().weight.shape[0], len(processor.tokenizer))
...
class _QWen3_Vision(nn.Module):
    ...
    def __init__(self, config: Optional[dict] = None, **kwargs):
        ...
        self.model = copy.deepcopy(model.visual)
        ...
        print("loaded only vision model")
        # print("vocab/emb size:", model.get_input_embeddings().weight.shape[0], len(processor.tokenizer))
        # print("image_token_id:", model.config.image_token_id)
    ...
    def forward(
        ...
        """
        with torch.autocast("cuda", dtype=torch.bfloat16):
            vit_tokens, deep_stacked_tokens = self.model(
                hidden_states=pixel_values,
                grid_thw=image_grid_thw,
            )
            feat_map, flr = self.model.vit_tokens_to_featmap(
                vit_tokens,
                image_grid_thw,
            )
    ...
    def build_qwenvl_inputs(self, images, instructions, solutions=None, **kwargs):
        ...
        return batch_inputs['pixel_values'], batch_inputs['image_grid_thw']
        return batch_inputs.to(self.model.device)
