...
# VGGT prompt changes Qwen token positions, so do not use the shared Qwen feature cache.
unset NAVSIM_FEATURE_CACHE_ROOT
export NAVSIM_USE_FEATURE_CACHE=0
export NAVSIM_VGGT_CACHE_ROOT="${NAVSIM_VGGT_CACHE_ROOT:-${DRIVEDREAMER_ROOT}/navsim_feature_cache/vggt_query_train_v2_layer11_global_m195}"
export NAVSIM_VGGT_CACHE_STRICT="${NAVSIM_VGGT_CACHE_STRICT:-1}"
...
run_id="${RUN_ID:-${timestamp}-vggt-action-lr1e5-16g-bz_${bz}-ga_${gradient_accumulation_steps}-${split}}"
...
CUDA_VISIBLE_DEVICES="${visible_devices}" accelerate launch \
  ...
  --framework.vggt.enabled true \
  --framework.vggt.mode "${VGGT_MODE:-fixed_query}" \
  --framework.vggt.query_count "${VGGT_QUERY_COUNT:-63}" \
  --framework.vggt.teacher_dim 1024 \
  --framework.vggt.bridge_heads "${VGGT_BRIDGE_HEADS:-16}" \
  --framework.vggt.loss_weight "${VGGT_LOSS_WEIGHT:-0.1}" \
  --framework.vggt.cosine_weight "${VGGT_COSINE_WEIGHT:-1.0}" \
  --framework.vggt.smooth_l1_weight "${VGGT_SMOOTH_L1_WEIGHT:-0.1}" \
  --framework.vggt.relational_weight "${VGGT_RELATIONAL_WEIGHT:-0.05}" \
  --framework.vggt.planner_mask "${VGGT_PLANNER_MASK:-all_true}" \
  --framework.vggt.alignment_mask valid_mask \
  --framework.vggt.use_bridge true \
  --framework.vggt.use_context true \
