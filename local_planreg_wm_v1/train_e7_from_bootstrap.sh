...
: "${BOOTSTRAP_CHECKPOINT:?Set BOOTSTRAP_CHECKPOINT}"
planreg_require_file_unless_dry_run "${BOOTSTRAP_CHECKPOINT}"
planreg_launch e7_from_bootstrap "${seed}" \
  "agent.checkpoint_path=${BOOTSTRAP_CHECKPOINT}" trainer.params.max_epochs=8 \
  agent.world_model.enabled=true agent.ema.enabled=true agent.world_model.future_mode=correct agent.world_model.predictor_only=true \
