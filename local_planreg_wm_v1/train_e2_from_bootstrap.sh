...
planreg_launch e2_from_bootstrap "${seed}" \
  "agent.checkpoint_path=${BOOTSTRAP_CHECKPOINT}" \
  trainer.params.max_epochs=8 \
  agent.world_model.enabled=false agent.ema.enabled=false \
  agent.lr_args.planning_adapter_lr=1.0e-4 \
  agent.lr_args.future_predictor_lr=1.0e-4 \
  agent.lr_args.fusion_lr=5.0e-5 \
  ...
  agent.scheduler_args.warmup_ratio=0.03
