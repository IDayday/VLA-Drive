...
# Source: starVLA/model/framework/QwenOFT.py
...
class Qwenvl_OFT:
    ...
    def _build_qwen_batch(self, examples, instructions):
        ...
        if all((payload is not None for payload in cached)):
            if not self.qwen_visual_frozen:
                raise RuntimeError('Qwen visual fine-tuning requires raw images; a frozen Qwen feature cache would block gradients')
        ...
        (image_embeds, deepstack_embeds) = encode_qwen_images(self.qwen_vl_interface.model.model, pixel_values=qwen_inputs['pixel_values'], image_grid_thw=qwen_inputs['image_grid_thw'], freeze_visual=self.qwen_visual_frozen)
        if not self.qwen_visual_frozen and torch.is_grad_enabled() and image_embeds.requires_grad:
        ...
        return (input_ids, attention_mask, position_ids, positions, image_embeds, deepstack_embeds, qwen_inputs['image_grid_thw'].reshape(len(examples), len(examples[0]['image']), 3))
    ...
    def _compute_query_extension(self, last_hidden, token_positions, examples, *, input_ids=None, image_grid_thw=None):
        """Extension hook; the legacy framework has no auxiliary query source."""
        return {'action_context': None, 'context_mask': None, 'losses': {}, 'metrics': {}}
    ...
    def _condition_inference_action_queries(self, last_hidden, input_ids, action_queries, *, image_grid_thw=None, examples=None):
    ...
    def _attach_framework_outputs(self, result, action_loss, extension, planner_metrics):
        ...
        result['metrics'] = {**extension.get('metrics', {}), **{name: value for (name, value) in planner_metrics.items() if torch.is_tensor(value) and value.numel() == 1}}
        return result
    ...
    def forward(self, examples: List[dict]=None, accelerator=None, **kwargs) -> Tuple:
        ...
        (input_ids, attention_mask, position_ids, token_positions, image_embeds, deepstack_embeds, image_grid_thw) = self._build_qwen_batch(examples, instructions)
        ...
        query_extension = self._compute_query_extension(last_hidden, token_positions, examples, input_ids=input_ids, image_grid_thw=image_grid_thw)
        planner_metrics = {}
        agent_dino_loss = torch.tensor(0.0).to(text_embeds.device)
        if self.action_prompt_mode == 'minimal_agent':
            ...
            if all((payload is not None for payload in agent_dino_payloads)):
                ...
                teacher_masks = torch.stack(teacher_masks, dim=0)
                ...
                pred_features = self.agent_dino_head(mine_agent_queries.float())
                if teacher_masks.any():
                    agent_dino_loss = F.smooth_l1_loss(pred_features[teacher_masks], teacher_features[teacher_masks])
        ...
        if self.config.datasets.vla_data.load_act_data == 1:
            ...
            (action_queries, action_context, planner_metrics) = self._condition_action_queries(action_queries, query_extension)
            with torch.autocast('cuda', dtype=torch.float32):
                ...
                repeat_action_context = action_context.repeat(repeated_diffusion_steps, 1, 1) if action_context is not None else None
                if self.w_video_latent:
                    ...
                    video_token = None
                if self.mlp_head == 0:
                    action_loss = self.action_model(repeat_action_queries, repeat_actions, video_token, extra_context=repeat_action_context)
                else:
        ...
        if self.config.datasets.reward_data.load_reward_data:
            ...
            return self._attach_framework_outputs(result, action_loss, query_extension, planner_metrics)
        else:
    ...
    def predict_action_infer_1d(self, examples, **kwargs) -> np.ndarray:
        """
        ...
        action_queries = last_hidden.gather(dim=1, index=g_idx)
        (action_queries, action_context, _) = self._condition_inference_action_queries(last_hidden, input_ids, action_queries, image_grid_thw=qwen_inputs['image_grid_thw'], examples=examples)
        ...
        with torch.autocast('cuda', dtype=torch.float32):
            if num_trajectory_samples > 1:
                action_queries = action_queries.repeat(num_trajectory_samples, 1, 1)
                if action_context is not None:
                    action_context = action_context.repeat(num_trajectory_samples, 1, 1)
            if self.mlp_head == 0:
                pred_actions = self.action_model.predict_action(action_queries, extra_context=action_context)
            else:
                pred_actions = self.action_model(action_queries)
        normalized_actions = pred_actions.detach().cpu().numpy()
        if num_trajectory_samples > 1:
            normalized_actions = normalized_actions.reshape(num_trajectory_samples, B, *normalized_actions.shape[1:]).transpose(1, 0, 2, 3)
        if False:
            rgb_data = [example['2d_gen_data'] for example in examples]
