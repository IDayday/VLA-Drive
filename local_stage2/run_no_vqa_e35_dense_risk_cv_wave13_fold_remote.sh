...
# Run one authoritative Wave-13 fold.  This is intentionally a one-fold
# launcher so the five folds can be distributed across hosts without loading
# the 95 GiB dense observation cache five times on a memory-constrained host.
...
fold_root="${NO_VQA_WAVE13_FOLD_ROOT:-${repo_root}/reports/m0_independent_scorer_representation/no_vqa_e35_risk_cv_wave12_v1/folds}"
run_root="${NO_VQA_WAVE13_RUN_ROOT:-/mnt/project/DriveVLA-M0-stage2/runs/scorer_pdms93/no_vqa_e35_dense_risk_cv_wave13_v3_distributed}"
log_root="${NO_VQA_WAVE13_LOG_ROOT:-/mnt/project/DriveVLA-M0-stage2/runs/scorer_pdms93_logs/no_vqa_e35_dense_risk_cv_wave13_v3_distributed}"
poll_seconds="${NO_VQA_WAVE13_POLL_SECONDS:-30}"
...
for path in "${source_root}" "${label_root}" "${fold_root}/fold_${fold}.json"; do
  [[ -e "${path}" ]] || { echo "missing Wave-13 input: ${path}" >&2; exit 2; }
done
for root in "${private_root}" "${actor_root}"; do
  [[ -f "${root}/.complete" ]] || { echo "incomplete Wave-13 cache: ${root}" >&2; exit 2; }
done
...
if [[ -e "${output}" || -e "${log_file}" || -e "${marker}" ]]; then
  echo "Wave-13 fold output already exists; refusing overwrite: fold=${fold}" >&2
  exit 2
fi
...
while true; do
  ...
  echo "NO_VQA_WAVE13_FOLD waiting_for_gpu fold=${fold} gpu=${gpu} used_mib=${used} utc=$(date -u +%FT%TZ)"
  sleep "${poll_seconds}"
done
...
echo "NO_VQA_WAVE13_FOLD_STARTED fold=${fold} gpu=${gpu} host=$(hostname) python=$("${python_bin}" -c 'import sys; print(sys.version.split()[0])') utc=$(date -u +%FT%TZ)"
...
echo "NO_VQA_WAVE13_FOLD_COMPLETE fold=${fold} gpu=${gpu} host=$(hostname) utc=$(date -u +%FT%TZ)"
