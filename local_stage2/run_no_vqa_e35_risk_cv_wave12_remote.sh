...
# Wave-12: one fully locked scorer is trained on five disjoint, risk-stratified
# physical-log folds. Every fold uses final epoch 7; no per-fold best epoch is
# allowed to influence the all-log refit decision.
...
log_root="${NO_VQA_WAVE12_LOG_ROOT:-/root/scorer_pdms93_logs/no_vqa_e35_risk_cv_wave12_v1}"
poll_seconds="${NO_VQA_WAVE12_POLL_SECONDS:-30}"

gpu_ids=(0 1 2 3 4)
for path in "${source_root}" "${label_root}" "${fold_root}/index.json"; do
  [[ -e "${path}" ]] || { echo "missing Wave-12 input: ${path}" >&2; exit 2; }
done
for fold in 0 1 2 3 4; do
  [[ -f "${fold_root}/fold_${fold}.json" ]] || {
    echo "missing Wave-12 fold ${fold}" >&2
    exit 2
  }
done
for root in "${private_root}" "${actor_root}"; do
  [[ -f "${root}/.complete" ]] || { echo "incomplete Wave-12 cache: ${root}" >&2; exit 2; }
done
if [[ -e "${run_root}" || -e "${log_root}" ]]; then
  echo "Wave-12 output already exists; refusing overwrite" >&2
  exit 2
fi
...
while true; do
  ...
  (( ready == 1 )) && break
  echo "NO_VQA_WAVE12 waiting_for_gpus utc=$(date -u +%FT%TZ)"
  sleep "${poll_seconds}"
done
...
for fold in 0 1 2 3 4; do
  ...
  pids+=("$!")
  echo "NO_VQA_WAVE12_FOLD_STARTED fold=${fold} gpu=${gpu} pid=$!"
done

failure=0
for fold in 0 1 2 3 4; do
  if wait "${pids[${fold}]}"; then
    echo "NO_VQA_WAVE12_FOLD_COMPLETE fold=${fold}"
  else
    echo "NO_VQA_WAVE12_FOLD_FAILED fold=${fold}" >&2
    failure=1
  fi
done
(( failure == 0 )) || exit 1
touch "${run_root}/.wave12_folds_complete"
echo "NO_VQA_WAVE12_FOLDS_COMPLETE root=${run_root}"
