...
# Scorer-private representation wave over current four-camera tokens exported
# from the locally trained No-VQA M0 vision encoder.  The eight variants are
# fixed before any result from this wave is evaluated on Navtest.
...
run_root="${NO_VQA_WAVE6_RUN_ROOT:-/root/scorer_pdms93_runs/no_vqa_e35_multiview_wave6_v1}"
log_root="${NO_VQA_WAVE6_LOG_ROOT:-/root/scorer_pdms93_logs/no_vqa_e35_multiview_wave6_v1}"
...
names=(
  rawcombined_top16_hybrid_standard_actor_seed2
  rawcombined_top8_hybrid_topregret_actor_seed2
  rawcombined_top16_reference_q50_strict_actor_seed2
  rawcombined_top32_reference_q50_strict_actor_seed2
  rawprivate_top16_reference_q50_strict_actor_seed2
  rawcontextcombined_top16_reference_q50_strict_actor_seed2
  rawcombined_top16_reference_q50_balanced_actor_seed2
  rawcombined_top16_reference_q50_strict_noactor_seed2
)

for path in "${source_root}" "${label_root}" "${split_manifest}" "${current_actor_root}"; do
  [[ -e "${path}" ]] || { echo "missing wave-6 input: ${path}" >&2; exit 2; }
done
while [[ ! -f "${private_root}/.complete" ]]; do
  echo "NO_VQA_WAVE6 waiting_for_trainval_multiview_cache utc=$(date -u +%FT%TZ)"
  sleep "${poll_seconds}"
done
if [[ -e "${run_root}" || -e "${log_root}" ]]; then
  echo "wave-6 output already exists; refusing overwrite" >&2
  exit 2
fi
...
while true; do
  ...
  (( ready == 1 )) && break
  echo "NO_VQA_WAVE6 waiting_for_gpus utc=$(date -u +%FT%TZ)"
  sleep "${poll_seconds}"
done
...
for index in "${!names[@]}"; do
  ...
  case "${name}" in
    ...
    rawcombined_top16_reference_q50_balanced_actor_seed2)
      ...
      ;;
    rawcombined_top16_reference_q50_strict_noactor_seed2)
      variant_args+=("${reference_args[@]}" --m0-candidate-fusion --residual-top-k 16 --private-factor-weight 0 --current-actor-weight 0)
      ;;
    *)
      echo "unhandled wave-6 variant: ${name}" >&2
      exit 2
      ;;
  esac
  (
  ...
  pids+=("$!")
  echo "NO_VQA_WAVE6_TRAIN_STARTED gpu=${gpu} name=${name} pid=$!"
done

failure=0
for index in "${!pids[@]}"; do
  if wait "${pids[${index}]}"; then
    echo "NO_VQA_WAVE6_TRAIN_COMPLETE name=${names[${index}]}"
  else
    echo "NO_VQA_WAVE6_TRAIN_FAILED name=${names[${index}]}" >&2
    failure=1
  fi
done
(( failure == 0 )) || exit 1
touch "${run_root}/.wave6_complete"
echo "NO_VQA_WAVE6_COMPLETE run_root=${run_root}"
