...
# Fourth No-VQA scorer wave on rl-zt4 GPUs 0/2/4.  It isolates two
# top-selection objectives that were absent from the initial residual runs:
# oracle-vs-rest top-regret ranking and shortlist-scoped factor supervision.
...
log_root="${NO_VQA_WAVE4_LOG_ROOT:-/root/scorer_pdms93_logs/no_vqa_e35_scene_token_wave4_v1}"
...
if [[ -e "${run_root}" || -e "${calibrated_root}" || -e "${log_root}" ]]; then
  echo "wave-4 output already exists; refusing overwrite" >&2
  exit 2
fi

# An early validation-promoted Navtest diagnostic may briefly use GPU 0/2.
# Wait without interrupting it, then claim only the predeclared idle GPUs.
while true; do
  ...
  (( ready == 1 )) && break
  echo "NO_VQA_WAVE4 waiting_for_gpus utc=$(date -u +%FT%TZ)"
  sleep 15
done
...
common_args=(
  ...
  --seed 2
  --residual-top-k 8
  --current-actor-target-root "${current_actor_root}"
...
for index in "${!gpu_ids[@]}"; do
  ...
  variant_args=()
  case "${name}" in
    *allfactor_topregret0*)
      variant_args+=(--factor-loss-scope all --top-regret-weight 0)
      ;;
    *allfactor_topregret1*)
      variant_args+=(--factor-loss-scope all --top-regret-weight 1)
      ;;
    *topkfactor_topregret1*)
      variant_args+=(--factor-loss-scope topk --top-regret-weight 1)
      ;;
  esac
  (
  ...
  train_pids+=("$!")
  echo "NO_VQA_WAVE4_TRAIN_STARTED gpu=${gpu} name=${name} pid=$!"
done

failure=0
for index in "${!train_pids[@]}"; do
  if wait "${train_pids[${index}]}"; then
    echo "NO_VQA_WAVE4_TRAIN_COMPLETE name=${names[${index}]}"
  else
    status=$?
    echo "NO_VQA_WAVE4_TRAIN_FAILED name=${names[${index}]} status=${status}" >&2
    failure=1
  fi
done
(( failure == 0 )) || exit 1
...
for index in "${!calibration_pids[@]}"; do
  if ! wait "${calibration_pids[${index}]}"; then
    status=$?
    echo "NO_VQA_WAVE4_CALIBRATION_FAILED name=${names[${index}]} status=${status}" >&2
    failure=1
  fi
done
(( failure == 0 )) || exit 1
touch "${run_root}/.wave4_complete"
echo "NO_VQA_WAVE4_COMPLETE run_root=${run_root} calibrated_root=${calibrated_root}"
