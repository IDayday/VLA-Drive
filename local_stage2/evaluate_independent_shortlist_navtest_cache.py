...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional current-only M0-native visual-token cache. When set, "
            "the independent scorer uses it instead of released Q-Former tokens."
        ),
    )
...
def main() -> None:
    ...
    summary = {
        ...
        "observation_source": (
            "m0_native_current_multiview_tokens"
            if private_table is not None
            else "released_qformer_scene_tokens"
        ),
